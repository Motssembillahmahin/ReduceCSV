...
# Handle the file upload, process CSV, and return processed file
@app.post("/upload")
async def upload_file(file: UploadFile = File(...)):
    try:
        ...
        contents = await file.read()  # Read file content (bytes)
        data = pd.read_csv(io.BytesIO(contents))  # Convert to DataFrame

        # Print out the columns of the received CSV for debugging
        print(f"Columns in uploaded CSV: {data.columns.tolist()}")
